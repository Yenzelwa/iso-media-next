...
import { dislikeVideoService } from '@/src/services/videos';
import DislikeRequestSchema from '@/schemas/videos/DislikeRequest.schema.json';
import LikeResponseSchema from '@/schemas/catalog/LikeResponse.schema.json';

export async function POST(req: Request, { params }: { params: { id: string } }) {
  try {
    const body = await parseJson(req);
    validate(DislikeRequestSchema, body);
    
    const data = await dislikeVideoService(params.id, body);
